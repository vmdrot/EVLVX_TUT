...
namespace BGU.DRPL.SignificantOwnership.Utility
{
    public class ParseDBF
    {
        ...
        private static Encoding _charsEncoding = Encoding.ASCII;
        private static PoorMansCharsReplacer _dosCharsReplacer = null;
        public static Encoding CharsEncoding
        {
            get { return _charsEncoding; }
            set { _charsEncoding = value; }
        }

        public static PoorMansCharsReplacer DosCharsReplacer
        {
            get { return _dosCharsReplacer; }
            set { _dosCharsReplacer = value; }
        }
                ...
                for (int counter = 0; counter <= header.numRecords - 1; counter++)
                {
                    ...
                    foreach (FieldDescriptor field in fields)
                    {
                        switch (field.fieldType)
                        {
                            ...
                            case 'C': // String
                                {
                                    string rawStr = CharsEncoding.GetString(recReader.ReadBytes(field.fieldLen));
                                    if (DosCharsReplacer != null)
                                        row[fieldIndex] = DosCharsReplacer(rawStr);
                                    else
                                        row[fieldIndex] = rawStr;
                                }
                                break;
    ...
    public delegate string PoorMansCharsReplacer(string strIn);
}